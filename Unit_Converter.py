...
class UnitConverterApp:
    def __init__(self, root):
        ...
        self.unit_type_menu = tk.OptionMenu(
            root, self.unit_type_var, *self.units.keys()
        )
        ...
        self.from_unit_menu = tk.OptionMenu(
            root, self.from_unit_var, *self.units[self.unit_type_var.get()].keys()
        )
        ...
        self.to_unit_menu = tk.OptionMenu(
            root, self.to_unit_var, *self.units[self.unit_type_var.get()].keys()
        )
        ...
        self.convert_button = tk.Button(
            root, text="Convert", command=self.convert_units
        )
        ...
        # Attach a trace to update unit menus when the unit type is changed
        self.unit_type_var.trace("w", self.update_unit_menus)

    def update_unit_menus(self, *args):
        # Update the available units in the "From Unit" and "To Unit" menus based on the selected unit type
        ...
        for unit in from_units:
            self.from_unit_menu["menu"].add_command(
                label=unit, command=tk._setit(self.from_unit_var, unit)
            )
        for unit in to_units:
            self.to_unit_menu["menu"].add_command(
                label=unit, command=tk._setit(self.to_unit_var, unit)
            )
    ...
    def convert_units(self):
        ...
        conversion_factor = (
            self.units[selected_unit_type][to_unit]
            / self.units[selected_unit_type][from_unit]
        )
        converted_amount = amount * conversion_factor

        # Display the result
        self.result_label.config(
            text=f"{amount} {from_unit} is {converted_amount:.2f} {to_unit}"
        )
